...
class AIController implements Comparable<AIController> {
  ...
  AIController () {
    ...
    Layers.add(new Matrix(8, 5));
    Layers.add(new Matrix(5, 5));
  ...
  int compareTo(AIController b) {
    ...
    float aPoints = PVector.sub(car.position, vaga.pos).mag(); if (vaga.checkCollision(car)) aPoints -= 500000;
    float bPoints = PVector.sub(b.car.position, vaga.pos).mag(); if (vaga.checkCollision(b.car)) bPoints -= 500000;
      
    println(aPoints, bPoints);
